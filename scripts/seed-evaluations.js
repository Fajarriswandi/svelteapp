...
if (!supabaseUrl || !supabaseServiceKey) {
	...
	console.error(
		'di file .env Anda. Ingat, SUPABASE_SERVICE_KEY (Service Role Key) TIDAK boleh diekspos ke publik.'
	);
	console.error('----------------------------------------------------');
	process.exit(1);
}
...
// --- Konfigurasi Rentang Tanggal untuk Data Dummy ---
// Sesuaikan tahun ini agar data dummy Anda berada dalam rentang yang relevan
// Misalnya, jika Anda ingin data untuk 2024 dan 2025, set START_YEAR = 2024 dan END_YEAR = 2025
const START_YEAR = 2024; // Ubah ini untuk menentukan tahun awal data dummy
const END_YEAR = 2025; // Ubah ini untuk menentukan tahun akhir data dummy
const NUMBER_OF_EVALUATIONS = 200; // Jumlah total data evaluasi yang akan dibuat
// ----------------------------------------------------

/**
 * Menghasilkan data acak untuk satu entri evaluasi.
 * Tanggal evaluasi akan berada dalam rentang START_YEAR dan END_YEAR.
 * @param {number} index - Indeks untuk membantu pembuatan data unik (opsional).
...
const generateRandomData = (index) => {
	...
	const aiScore = Math.floor(Math.random() * (100 - 70 + 1)) + 70; // Skor AI antara 70-100
	const humanRating = (Math.floor(Math.random() * (50 - 30 + 1)) + 30) / 10; // Rating 3.0 - 5.0

	// Logika baru untuk menghasilkan tanggal dalam rentang tahun yang ditentukan
	const startOfYear = new Date(START_YEAR, 0, 1); // 1 Januari START_YEAR
	const endOfYear = new Date(END_YEAR + 1, 0, 1); // 1 Januari (END_YEAR + 1) untuk mencakup seluruh END_YEAR
	const randomTimestamp =
		startOfYear.getTime() + Math.random() * (endOfYear.getTime() - startOfYear.getTime());
	const date = new Date(randomTimestamp); // Tanggal acak dalam rentang
	...
	return {
		...
		evaluation_date: date.toISOString().split('T')[0], // Format YYYY-MM-DD
		ai_score: aiScore,
...
const generateEvaluationDetails = (evaluationId) => {
	const categories = [
		{
			key: 'accuracy',
			name: 'Accuracy of Response',
			question:
				"Did the AI understand the user's intent correctly? Was the answer factually and contextually correct?",
			icon: 'target'
		},
		{
			key: 'relevance',
			name: 'Relevance & Coherence',
			question: 'Did the response stay on-topic? Was it logically coherent?',
			icon: 'chat-quote'
		},
		{
			key: 'tone',
			name: 'Tone & Politiness',
			question: 'Was the tone appropriate (e.g., friendly, professional)?',
			icon: 'headset'
		},
		{
			key: 'resolution',
			name: 'Resolution Effectiveness',
			question: "Did the conversation solve the user's issue?",
			icon: 'patch-check'
		},
		{
			key: 'escalation',
			name: 'Escalation Appropriateness',
			question: 'Was a human handover triggered when needed?',
 ...
 * Penting: created_at di sini akan sama dengan evaluation_date dari evaluasi induk.
 * @param {string} evaluationId - ID dari evaluasi induk.
 * @param {Date} evaluationDate - Objek Date dari evaluation_date induk.
 * @returns {Array<object>} Array objek riwayat evaluasi.
 */
const generateEvaluationHistory = (evaluationId, evaluationDate) => {
	const histories = [
		{
			type: 'chat',
...
[09:15] AI Agent: Glad to help that! Let us know if you need anything else.`
	...
	// Pastikan created_at di evaluation_history juga menggunakan tanggal dari evaluation induk
	return histories.map((h) => ({
		evaluation_id: evaluationId,
		type: h.type,
		content: h.content,
		created_at: evaluationDate.toISOString() // Menggunakan tanggal evaluasi induk
	}));
};
...
const seedData = async () => {
	console.log('----------------------------------------------------');
	console.log(
		`Memulai proses seeding data evaluations, details, dan history untuk tahun ${START_YEAR}-${END_YEAR}...`
	);
	console.log('----------------------------------------------------');
	...
	const { error: deleteHistoryError } = await supabase
		...
		.delete()
		.neq('id', '00000000-0000-0000-0000-000000000000'); // Menghapus semua kecuali ID dummy
	...
	const evaluationsToInsert = Array.from({ length: NUMBER_OF_EVALUATIONS }, (_, i) =>
	...
	const { data: insertedEvaluations, error: insertEvaluationsError } = await supabase
		...
		.select('id, evaluation_date'); // PENTING: Minta ID dan evaluation_date dari baris yang baru disisipkan
	...
	if (insertedEvaluations) {
		for (const evaluation of insertedEvaluations) {
			...
			// Generate history, meneruskan objek Date dari evaluation_date
			const historyDate = new Date(evaluation.evaluation_date); // Konversi string kembali ke objek Date
			const histories = generateEvaluationHistory(evaluation.id, historyDate);
